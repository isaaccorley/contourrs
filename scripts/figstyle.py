...
CATEGORICAL = tuple(
    COLORS[n] for n in ("coral", "blue", "green", "ochre", "purple", "teal")
)
WARM = LinearSegmentedColormap.from_list(
    "warm_magnitude", [COLORS["ivory"], ACCENT, RELATED]
)
SIGNED = LinearSegmentedColormap.from_list(
    "signed_residual", [COLORS["blue"], COLORS["ivory"], ACCENT]
)
TEXT_WIDTH = 5.5
...
def apply_style() -> str:
    ...
    if chosen not in candidates[:2]:
        warnings.warn(
            f"Figure font fallback: {chosen}; inspect its rendered metrics.",
            stacklevel=2,
        )
    mpl.rcParams.update(
        {
...
def tint(color: str, strength: float) -> tuple[float, float, float]:
    ...
    if not 0 <= strength <= 1:
        message = "Tint strength must be between zero and one."
        raise ValueError(message)
    red, green, blue = (1 - strength * (1 - channel) for channel in to_rgb(color))
...
def cell_text_color(background) -> str:
    ...
    def luminance(color):
        ...
        linear = [
            c / 12.92 if c <= SRGB_LINEAR_CUTOFF else ((c + 0.055) / 1.055) ** 2.4
            for c in channels
        ]
        return sum(
            c * weight
    ...
    return (
        choice
        if contrast(choice) >= MIN_TEXT_CONTRAST
        else max(("black", "white"), key=contrast)
    )
